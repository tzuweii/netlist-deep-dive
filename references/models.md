...
改用**枚舉候選封裝、以證據排名**。三個獨立證據來源：

| 來源 | 標記 | 說明 |
|---|---|---|
| **拓樸一致性** | `[N 板名 refdes.pin]` | 模型宣告的電源/接地腳，實際是不是接在電源/地上 |
| 料號後綴 | `[B 板名 refdes]` | 訂購碼的封裝碼（`…PW` / `…BS`） |
| footprint 名稱 | `[N 板名 refdes.pin]` | layout 選的實體 footprint |
...
`boundary(not_followed)` 只在板內追蹤出現：停在**本板這一側**的腳位上，對面是哪支腳寫在原因欄，對面的零件不進路徑。

⚠️ **`n_loads` / `loads` 是「訊號抵達的腳位」，不是「已確認的負載」。**
`unclassified` 也算在裡面（見上表），所以那個數字包含「還沒確認是不是負載」
的腳位。要區分請看同一列的 `endpoint_kind` 欄。這是刻意的取捨——排除它會讓
輸出幾乎空掉（見下），但引用那個數字時不能講成「N 個負載」。
