...
`MISSING.md` / `coverage` 的「無」欄只比**料號前 6 碼 vs 檔名**，family datasheet
排第二個以後的型號一律漏判（實測 `ADG919BCPZ` 被判成缺，`ADG918_919.pdf` 就在
資料夾裡）。所以：

| 來源說 | 怎麼用 |
|---|---|
| `MISSING.md` 說**有** | 可信，直接往下走（但仍要 `pinfn` 開檔對名才知道腳位功能） |
| `MISSING.md` 說**缺** | **不可信**，先用 `pinfn` 複核 |

`pinfn` 的找檔是三層後援：料號前綴 → 型號字母段 → **全文搜尋**（會剝除常見訂購碼
後綴，`ADG919BCPZ` → 也試 `ADG919`）。比檔名比對強得多，但它自己也有三個**靜默**
失敗點，`pinfn` 說找不到時要自己開資料夾確認：

- 全文搜尋**只掃前 3 頁**——型號只出現在末尾訂購表的會漏
- **掃描檔**（圖檔 PDF）抽不出文字，回空字串
- 環境沒裝 `pypdf` 時第三層**整層停用**，退化成只比檔名，不會警告

真正的保險不在找檔那一步，而在 `pinfn` 的**自動對名**：抽出來的腳位名對不上
`.DSN` symbol 就當場攤開、不寫快取。所以「`pinfn` 給的答案」可信，
「`pinfn` 說找不到」不可信。

確認資料夾裡真的沒有，才寫 `[D 缺]`，並列進 `MISSING.md` 請使用者提供。
