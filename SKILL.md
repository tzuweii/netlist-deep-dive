...
| 主張類型 | 唯一合法來源 | 標記（**一律帶受詞**） |
|---|---|---|
| 連線——誰接到誰 | netlist（`.asc`） | `[N: <板> U913.2 = TX_EN_P_0]` |
| 身分——料號、值、**BOM 範圍內有無列出** | BOM | `[B: <板> U913 = PCA9554BBSHP]`／`= 未貼件` |
| 腳位功能、方向、內部行為、極性 | datasheet | `[D 檔名 p.x]` |
| **腳位的功能名**（只有名字）、**低有效**、**零件在哪個子電路** | `.DSN` 的 Capture symbol 與階層 | `[S: <檔>.DSN U913 pin2 = P0]` |
| 推論 | 上述組合 + 寫出推理過程 | `[?: <主張> —— 定案方式：<怎麼拍板>]` |

### 引用一律帶受詞——**不帶受詞的標記等於沒標**

裸 `[N]` 沒查過也打得出來，**在原理上無法被戳破**；`[N: fecu_fm U913.2 =
TX_EN_P_0]` 一行指令就驗得掉。這個格式管不住我，但它把「無法證偽」換成
「可證偽」，而那是使用者唯一能自保的地方。

⚠️ **`[N]`／`[B]` 的板名必填。** 這是唯一一格「受詞不完整會主動誤導」的欄位：
`U28 (TPS3702CX18DDCR)` 在 DPU 板完全正確，但同一塊專案的 Interposer 上
`U28` 是光耦。少了板名不是驗不了，是**把人帶去錯的零件**。

`[?]` 沒有來源可引，所以它的受詞是**定案方式**——這樣每一條推論自動變成一張
待辦，而不是一句免責聲明。

### 一次查詢 = 一個引用（**不是**一顆 refdes = 一個引用）

引用的數量跟「查了幾個獨立來源」成正比，**不跟講了幾顆零件成正比**。一條 20
跳的鏈，受詞是那一行指令：

```
[N: ndd.py trace --board fecu_fm --from U913.2]
```

比列 20 個 refdes 更強：更短、使用者跑一次就拿到完整原文、而且 netlist 改版後
貼死的清單會默默過期，指令不會。

**鏈路本身不必逐顆引用；要引用的是「工具輸出之外我多加的東西」**——穿越件的
致能腳（`[D]`）、這條線的用途（`[?]`）。典型一條鏈 3–4 個引用就結束。

若某條鏈真的需要幾十個引用，那代表正在把機器輸出改寫成散文——**那件事本身就
不該做**，直接貼工具輸出。
...
證據表**每列給編號**（`E1`、`E2`…），內文的標記寫成 `[N:E3]` 指回去。這擋的是
**孤兒表格**——表格查得漂漂亮亮、結論卻沒有用到那幾列。每列至少要被內文引用
一次，否則就是擺著好看的。

⚠️ **這套格式只約束新寫的內容，既有文件不回填。** 實測既有的四份架構文件共
820 條可機械查證的引用、事實錯誤 0——回填幾百行買到的是零。

### 這套格式**不解決**的兩件事（要講出來，不要假裝解決了）

1. **引用全對、結論錯。** 表格每列都重跑成立，但「所以這條是 SPI 時脈」是推
   出來的。機器驗不到，這正是 `[?]` 那幾列要留給人審的原因——所以 `[?]` 要
   可數、要帶定案方式。
2. **選擇性引用。** 只引成立的三條、略過會推翻結論的第四條，表格全綠。缺的那
   條對人與機器都是隱形的。使用者對結論起疑時的解法是**繞過我的取捨**：直接
   `ndd.py pins <refdes>` 把整顆零件的腳位攤開自己看。

### ⚠️ 用工程語言回答，不要貼工具欄位名

使用者要的是電路結論，**不是腳本的內部狀態**。來源標記要保留（位置固定、可掃；
完整受詞放證據表，內文用 `[N:E3]` 這種短形式指回去），但**工具的欄位名一律翻譯
成一句話**：
...
證據表也一樣——「來源」欄寫**帶受詞的引用**（`[N: fecu_fm U913.2 = TX_EN_P_0]`、
`[D tps3701.pdf p.5]`），不要寫工具欄位名。
