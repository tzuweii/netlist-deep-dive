...
**引用寫在句子裡，不要另外做一張證據表**——受詞已經在句子裡了，表格是同一份
資訊再抄一次。**沒有標記的主張，依定義就是未查證的**。

回答結尾列一行**這次查了什麼**，並主動寫出「這次我沒查什麼」：

```
查過：net TX_EN_P_0、trace --board fecu_fm --from U913.2、pins U937、ls datasheets/
```
