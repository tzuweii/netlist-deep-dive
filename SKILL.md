...
回答結尾附一段證據，**按類型分組**，不逐句編號、不做表格。**組內每一條照樣
用方括號帶受詞**——位置搬到結尾，語法不變：

```
連通性：[N: fecu_fm U913.2 = TX_EN_P_0]（全網只接兩支腳）
        [N: ndd.py trace --board fecu_fm --from U913.2]（追出兩條分支）
身分：  [B: fecu_fm U913 = PCA9554BBSHP]
        [B: fecu_fm U937 = SN74CBTLV3126DGVR]
腳位功能：[N: fecu_fm U937.13 = VDD_3V3_P]（選擇腳綁電源）
        [D 缺]（導通/斷開極性未查，SN74CBTLV3126 沒有規格書）
推論邊界：[?: 選擇腳綁電源代表常通 —— 定案方式：補 SN74CBTLV3126 規格書]
```

方括號語法保留是刻意的，跟「不要散在敘述裡」是兩件獨立的事：語法讓每一條
可以被 `grep` 找到，也讓「板名」變成模板裡固定的一格——漏寫會**明顯破格**，
改寫成散文就只是「這句話寫得比較隨性」，看不出來是漏了。

**每一行自己就要完整**，不靠回原句對照——板名、refdes、pin、net 都寫在這
一行的方括號裡，讀的人不必跳回敘述找受詞。⚠️ **`[N]`／`[B]` 板名必填**：
同一顆 `U28` 在 DPU 板是電壓監視器，在 Interposer 是光耦，少了板名會**把
人帶去錯的零件**，不只是驗不了。
