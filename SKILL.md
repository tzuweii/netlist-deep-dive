...
| 推論 | 上述組合 + 寫出推理過程 | `[?: <主張> —— 目前依據：E3 —— 定案方式：<怎麼拍板>]` |
...
`[?]` 沒有來源可引，所以它的受詞是**目前依據 + 定案方式**——這樣每一條推論自動
變成一張待辦，而不是一句免責聲明。詳見下面的〈`[?]` 是有依據的假設〉。

### 一個獨立查詢結果 = 一個引用單位（**不是**一顆 refdes = 一個引用）
...
一個 `[?]` 至少要講三件事：**主張**、**目前依據**、**定案方式**。允許寫成一行：

```
[?: NET_A = SPI_SCLK —— 目前依據：E3 —— 定案方式：查 datasheet 確認 pin7 功能]
```

**「目前依據」指證據表的編號，不在 `[?]` 裡巢狀放 `[S]`／`[D]`。** 受詞集中在
證據表那一列（`E3 [S: fecu_fm.DSN U12 pin7 = SCLK]`），`[?]` 保持精簡，同一個
定位資訊也不會在兩個層級各寫一次。指向的那列**照樣要守 source boundary**——
腳位**名字**是 `[S]`，腳位**行為**是 `[D]`，不因為 `[?]` 是新欄位就放寬。

**`目前依據：無` 是合法狀態。** 沒有資料支持不代表不能提出方向——

```
[?: U7 可能是備援路徑的切換點 —— 目前依據：無 —— 定案方式：問本板負責人]
```

「無」本身就是重要訊息（這是純猜測，讀的人可以直接跳過），比閉嘴或硬湊一列弱
證據都好。**禁止的是把沒有依據的假設寫成確定結論**：`目前依據：無` 的內容只能
留在 `[?]` 裡，不得在摘要、架構描述或結論中以確定語氣出現。

**已經查到的反向證據，用具名欄位寫出來**（沒有就整格不出現）：

```
[?: <主張> —— 目前依據：E3 —— 反向證據：E8 —— 定案方式：<如何確認>]
```
...
證據表**每列給編號**（`E1`、`E2`…），內文的標記寫成 `[N:E3]` 指回去。證據表
分兩區，**編號跨兩區連續編**（`[?]` 的「目前依據」可能指到任一區）：

| 區 | 收什麼 | 內文要引用嗎 | 成本控制 |
|---|---|---|---|
| **支撐結論** | 直接撐住最終結論的證據 | ✅ 要 | ✅ **只限這一區**：同一個查詢結果可以撐多個主張，不必重複建列 |
| **查過沒用上** | 這輪實際跑過、且直接涉及結論所涉的**同一顆零件或同一條 net** 的查詢 | ❌ 不用 | ❌ 不適用 |

⚠️ **「查過沒用上」符合條件就一律收錄，沒有裁量空間。** 這一區存在的唯一理由
是壓縮我選擇性省略的空間——收不收由我決定的話，不方便的那筆就會剛好沒被收
錄，規則當場歸零。**不得以「查完發現沒影響結論」為由刪除。**

這一區**只記那一行查詢指令，不貼完整輸出**（輸出重跑就有，`coverage`、長 trace
的輸出會把表撐爆）：

```
E8 [查過沒用上] ndd.py --board fecu_fm pins U913
```

⚠️ 指令要**能直接跑**。`--board` 是全域參數，放在子指令**前面**；`pins` 吃位置
參數，沒有 `--refdes`（那是 `pinfn` 的旗標）。規則文件裡貼不能跑的指令會被照抄，
而那正好摧毀這套格式要建立的東西。
...
| 目前依據 | 揭露這個假設**目前站在哪裡** |
| 反向證據 | 揭露**已知**的限制或反例 |
| 定案方式 | 指出還沒確定時**下一步怎麼確認** |

只有「查過沒用上」是符合條件就強制收錄的硬規則；`目前依據：無` 是合法狀態；
「反向證據」只有實際存在時才出現。
...
1. **引用全對、結論錯。** 表格每列都重跑成立，但「所以這條是 SPI 時脈」是推
   ...
   可數、要帶目前依據與定案方式。
2. **漏查的反向證據。** 要分清楚兩件事：**已經查到卻不講**，是揭露義務，上面
   的「反向證據」欄位與「查過沒用上」區管得到；**根本沒去查**，任何格式規則都
   保證不了。所以這兩樣是**強制揭露紀錄，不是選擇性引用的自動偵測**。使用者
   懷疑還有沒被揭露的東西時，解法仍然是**繞過我的取捨**——自己跑
   `ndd.py --board <板> pins <refdes>`、`trace` 把原始事實攤開看。
